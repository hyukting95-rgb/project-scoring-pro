...
-- 3. 暂停用户
DROP FUNCTION IF EXISTS public.suspend_team_member(UUID);
...
-- 4. 激活用户
DROP FUNCTION IF EXISTS public.activate_team_member(UUID);
...
-- 验证
SELECT proname, proargtypes FROM pg_proc WHERE proname LIKE '%team_member%' OR proname = 'get_member_by_id';
